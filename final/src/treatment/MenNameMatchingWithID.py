...
men_players_fix.write("player_id,name,hand,date_of_birth,country,height\n")
...
for ind in names.index:
    ...
    for ind2 in men_players.index:
        ...
        if((sobrenome == sobrenome2 or segsobrenome2 ==sobrenome) and inicial == inicial2):
            ...
            men_players_fix.write(str(men_players["player_id"][ind2])+","+names["PlayerName"][ind]+","+men_players["hand"][ind2]+","+str(men_players["dob"][ind2])+","+men_players["ioc"][ind2]+","+str(int(height))+"\n")
            breaked = 1
            height = meanheight
            break
    if(breaked == 0):
